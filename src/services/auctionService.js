...
export function getAuctionWorkspace({ activeTenantContext, groups = [], members = [] } = {}) {
  const auctions = AuctionRepository.list(activeTenantContext);
  const winners = listWinnerResults(activeTenantContext);
  return { auctions, winners, groups, members };
}

export function buildAuctionPreview({ activeTenantContext, group, members = [], monthNumber = 1, bidAmount = 0, bidPercentage = 0, winnerId = "" } = {}) {
  const scheduleRow = ChitScheduleRepository.listByGroup(group?.id, activeTenantContext).find((row) => Number(row.monthNumber) === Number(monthNumber)) ||
    { monthNumber, standardPayment: group?.monthly_amount, nonLiftedPayment: group?.monthly_amount, winnerSelectionMode: "AUCTION", isUserConfirmed: true };
  const ruleSet = ChitRuleRepository.getByGroup(group?.id, activeTenantContext) || { id: "legacy-rule", paymentPatternType: "FIXED", liftEffectiveRule: "NEXT_MONTH", winnerLockRule: "ONCE_LIFTED_LOCKED", minimumBidType: "PERCENTAGE", minimumBidValue: 0, maximumBidType: "PERCENTAGE", maximumBidValue: 100, commissionType: "PERCENTAGE", commissionValue: 5 };
  const winners = listWinnerResults(activeTenantContext);
  ...
  const preview = AuctionEngine.buildAuctionPreview({ group, scheduleRow, ruleSet, bidAmount, bidPercentage });
...
export function confirmAuctionWinner({ activeTenantContext, group, members = [], monthNumber = 1, bidAmount = 0, bidPercentage = 0, winnerId = "", userId = "local-user" } = {}) {
  ...
  const auction = AuctionRepository.save({
