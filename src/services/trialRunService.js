...
function buildSteps(context) {
  ...
  return [
    ...
    createStep(14, "Record organizer-decided bid", "/chits/manual-records", getStatus(auctionReady, groupActiveOrArchived), "Organizer-entered decision record exists."),
    createStep(15, "Record declared recipient", "/chits/manual-records", getStatus(context.hasWinner, auctionReady), "Organizer-declared recipient is recorded for Month 1."),
    createStep(16, "Record final distribution amount", "/chits/distributions", getStatus(Number(auction.prize_amount || 0) > 0, context.hasWinner), "Organizer-entered distribution amount is stored."),
    createStep(17, "Verify recorded details", "/chits/manual-records", getStatus(Number(auction.discount_amount || 0) > 0, context.hasWinner), "Stored record matches the organizer-entered details."),
