...
test("AI router uses deterministic routes and reports unconfigured providers",()=>{const result=new AIOrchestrator().route({text:"show pending",tenantContext:tenant});assert.equal(result.action.route,"/chits/collections/pending");assert.equal(result.providerStatus.ai.configured,false);assert.equal(result.providerStatus.ocr.configured,false)});
