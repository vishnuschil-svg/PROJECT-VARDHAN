...
import { RAZORPAY_SERVER_VARIABLES, validateRazorpayEnvironment } from "./razorpay-env-contract.mjs";
...
for (const variable of ["DATABASE_URL", "SUPABASE_JWT_SECRET", "REDIS_URL", "WHATSAPP_ACCESS_TOKEN", "SMS_GATEWAY_API_KEY", "EMAIL_API_KEY", ...RAZORPAY_SERVER_VARIABLES, "LICENSE_SIGNING_SECRET", "BACKUP_ENCRYPTION_KEY", "MONITORING_ALERT_WEBHOOK"]) requireMatch(envExample, new RegExp(`^${variable}=`, "m"), `missing environment contract ${variable}`);
...
const runtimeProduction = String(process.env.VARDHAN_ENV || "").trim().toLowerCase() === "production";
if (runtimeProduction) failures.push(...validateRazorpayEnvironment(process.env, { requireValues: true, production: true }).failures);
