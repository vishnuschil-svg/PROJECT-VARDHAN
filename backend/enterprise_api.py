...
def build_enterprise_router(workspace_dependency: Callable[..., Any], pool_getter: Callable[[Request], Any]) -> APIRouter:
    ...
    @router.post("/billing/subscriptions", deprecated=True)
    async def deprecated_create_subscription() -> dict[str, Any]:
        raise HTTPException(status.HTTP_410_GONE, "Use /api/payments/razorpay/create-order")
    ...
    @router.post("/webhooks/razorpay", deprecated=True)
    async def deprecated_razorpay_webhook() -> dict[str, Any]:
        raise HTTPException(status.HTTP_410_GONE, "Use /api/payments/razorpay/webhook")
